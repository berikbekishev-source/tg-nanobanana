...
5.  **[ROOT CAUSE]** Исправлен конфликт регистрации обработчиков:
    - Проблема: 3 обработчика (Sora, Kling, Veo) имели одинаковый декоратор `@router.message(StateFilter("*"), F.web_app_data)`
    - Aiogram выполняет только ПЕРВЫЙ подходящий обработчик → Sora перехватывал ВСЕ запросы
    - Решение: создан единый dispatcher `handle_webapp_data_dispatcher`, который роутит по полю `kind`
    - Коммит 2b24212c работал, потому что Sora еще не был добавлен

### Цель:
Увидеть ошибку на клиенте (через alert) или заставить запрос пройти.

### Результат:
✅ Fix deployed. Kling WebApp должен работать после деплоя.
