...
## [2025-12-04] Токенизация промта по референсу

**Агент:** Codex  
**Ветка:** worktree/staging-latest-20250209  
**PR:** не создан (локальная работа)

### Действия:
1. Добавлена миграция `0043_add_reference_prompt_model`: модель `gemini-2.5-pro` с base_cost_usd=0.01, cost_unit=generation, скрыта из общих списков.
2. В референс-флоу выводится стоимость и выполняется списание через BalanceService перед генерацией промта (с проверкой баланса и возвратом на ошибках).
3. Финальное сообщение промта включает списание/остаток токенов и сам промт; стартовое приглашение обновлено под ссылки/видео/изображения с ценой.
4. Отправка видео в Telegram переведена на plain caption (parse_mode отключен), чтобы исключить 400 от Telegram из-за неэкранированных спецсимволов.

### Результат:
Списание токенов за промт по референсу настроено, тексты и фолбэки обновлены; готово к проверке.
