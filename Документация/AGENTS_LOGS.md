## [2025-12-08] Kling (useapi): убран лишний maxJobs
...
### Результат:
- Payload text2video/image2video соответствует документации useapi; вероятность 400 из-за неподдерживаемых параметров снижена. Готово к пушу после проверки.
...
### Выполненные действия:
1. Добавил миграцию 0042_update_kling_model: переименовал модель Kling (slug/name/api_model/display_name) на `kling-v2-5-turbo` / `🎥 Kling v2-5-turbo`, деактивировал старую запись при наличии дублирующей.
2. Обновил бота и WebApp: новый slug в обработчике Kling, заголовок и дефолтный modelSlug в Kling WebApp, модель в списке цен `🌀 Kling v2-5-turbo`, поправил тесты.
3. Привёл документацию и model_pricing.csv к новому названию модели.

### Результат:
- PR #464 автосмержен в `staging`, деплой web/worker/beat успешен на коммите `222f341c884785f34eb1ced1b4c51ce99917293c`.
...
**Агент:** Codex  
**Ветка:** feature/new_veo_provider  
**PR:** #454 (MERGED, supersedes #452-453)

### Выполненные действия:
1. В WebApp/handler Veo читаю выбранное разрешение из payload и валидирую по allowed_resolutions модели, чтобы 1080p не сбрасывалось на 720p.
2. В обработчике Sora валидирую resolution по разрешённым значениям, избегая принудительного отката на дефолт.
3. В сообщениях о завершении генерации разделяю видео/изображения, чтобы формат не подменялся разрешением и выводился выбранный размер видео.

### Результат:
- PR #454 смержен в staging, деплой web/worker/beat успешен (commit 4ca76684448d513d711f3e03d84ddaa25c9ed8cc).
- Ожидается корректное отображение и фактическая генерация 1080p для Veo 3.1 Fast и Sora-2 при выборе пользователем.
...
## [2025-12-04] Тарификация «Промт по референсу»

**Агент:** Codex  
**Ветка:** agent-codex-staging-latest-67  
**PR:** не создан (локальная работа)

### Действия:
1. Добавлена миграция с моделью тарификации `gemini-2.5-pro` (generation, base_cost_usd=0.01, display_name «📲Промт по референсу», провайдер gemini, order=990).
2. Флоу «Промт по референсу»: показ цены в стартовых сообщениях, поиск модели тарификации и проверка баланса, предсписание токенов с возвратом при ошибке, финальное сообщение с списанными/оставшимися токенами и промтом.
3. Добавлены константа slug и хелперы для цены, промт отправляется без parse_mode для избежания 400 Telegram.
4. Исправлено создание пользователя в reference prompt (get_or_create + ensure_balance) чтобы не падать на новых пользователях.
5. Перестали создавать новый профиль: используем существующий TgUser по chat_id/from_user, иначе просим пройти /start.
