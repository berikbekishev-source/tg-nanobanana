# Интеграция OpenAI Sora 2

## Переменные окружения

Добавьте в Railway и локальные `.env` следующие значения:

- `OPENAI_API_KEY` — ключ доступа к OpenAI (не коммитим).
- `OPENAI_API_BASE` — при необходимости укажите кастомный endpoint (по умолчанию `https://api.openai.com/v1`).
- `OPENAI_ORGANIZATION` — идентификатор организации, если требуется.
- `OPENAI_VIDEO_POLL_INTERVAL` и `OPENAI_VIDEO_POLL_TIMEOUT` — при необходимости переопределите интервалы опроса задач.
- `OPENAI_VIDEO_REQUEST_TIMEOUT` и `OPENAI_VIDEO_CONTENT_TIMEOUT` — глобальные таймауты запросов (в секундах).

## Использование

1. Активируйте модель `Sora 2 (OpenAI)` в админке или выполните миграцию `0013_add_openai_sora_model`.
2. Выдайте доступ нужным пользователям (модель помечена как beta).
3. Для генерации доступны режимы `text2video` и `image2video`. Продление (`extend`) пока работает только для Veo.
4. При ошибках API провайдер возвращает понятные сообщения (429 — квоты, 403 — нет доступа и т.п.), они отображаются пользователю и логируются.

## Лимиты и тарификация

OpenAI не публикует фиксированную стоимость для Sora 2. Тарифы и квоты назначаются индивидуально в Dashboard. Перед запуском проверяйте:

- Длительность: до 60 секунд.
- Разрешения: 720p и 1080p (вертикальное 9:16 и горизонтальное 16:9).
- Rate limits: обычно десятки задач в минуту на организацию.

При превышении лимитов бот сообщит о необходимости повторить запрос позже или увеличить квоту в OpenAI.
