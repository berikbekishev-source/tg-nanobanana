...
- Стейджинг/прод БД разнесены:
  ...
  - **Staging**: отдельный проект Supabase `tg-nanobanana-staging` (`srquwlfweefqzpowdtiw`, регион `eu-west-1`, хост `db.srquwlfweefqzpowdtiw.supabase.co`). `DATABASE_URL` хранится только в окружении Railway **staging** (пароль не публикуем в репо).
