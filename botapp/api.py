...
try:
    ...
    def _submit_webapp_to_celery(user_id: int, data: Dict[str, Any], endpoint_name: str) -> None:
        """
        Отправляет данные WebApp в Celery для фоновой обработки.
        WebApp закрывается мгновенно.
        """
        from botapp.tasks import process_webapp_submission
        process_webapp_submission.delay(user_id, data)
    ...
    def midjourney_webapp_submit(request):
        ...
        Обработка выполняется в Celery, WebApp закрывается мгновенно.
        """
        try:
            ...
            _submit_webapp_to_celery(int(user_id), data, "MIDJOURNEY")
            return JsonResponse({"ok": True})
    ...
    def midjourney_video_webapp_submit(request):
        ...
        try:
            ...
            _submit_webapp_to_celery(int(user_id), data, "MIDJOURNEY_VIDEO")
            return JsonResponse({"ok": True})
        except Exception as exc:
    ...
    def runway_webapp_submit(request):
        ...
        try:
            ...
            _submit_webapp_to_celery(int(user_id), data, "RUNWAY")
            return JsonResponse({"ok": True})
        except Exception as exc:
    ...
    def runway_aleph_webapp_submit(request):
        ...
        try:
            ...
            _submit_webapp_to_celery(int(user_id), data, "RUNWAY_ALEPH")
            return JsonResponse({"ok": True})
        except Exception as exc:
    ...
    def veo_webapp_submit(request):
        """
        Fallback endpoint for Veo WebApp data submission.
        Обработка выполняется в Celery, WebApp закрывается мгновенно.
        """
        try:
            payload = json.loads(request.body.decode("utf-8"))
            ...
            _submit_webapp_to_celery(int(user_id), data, "VEO")
            return JsonResponse({"ok": True})
        except Exception as e:
    ...
    def gpt_image_webapp_submit(request):
        """
        Fallback endpoint для GPT Image WebApp.
        Обработка выполняется в Celery, WebApp закрывается мгновенно.
        """
        try:
            payload = json.loads(request.body.decode("utf-8"))
            ...
            _submit_webapp_to_celery(int(user_id), data, "GPT_IMAGE")
            return JsonResponse({"ok": True})
        except Exception as exc:
    ...
    def sora2_webapp_submit(request):
        ...
        try:
            ...
            _submit_webapp_to_celery(int(user_id), data, "SORA2")
            return JsonResponse({"ok": True})
        except Exception as exc:
    ...
    def kling_webapp_submit(request):
        """
        Fallback endpoint для Kling WebApp.
        Обработка выполняется в Celery, WebApp закрывается мгновенно.
        """
        try:
            payload = json.loads(request.body.decode("utf-8"))
            ...
            _submit_webapp_to_celery(int(user_id), data, "KLING")
            return JsonResponse({"ok": True})
        except Exception as exc:
    ...
    def nano_banana_webapp_submit(request):
        ...
        try:
            ...
            _submit_webapp_to_celery(int(user_id), data, "NANO")
            return JsonResponse({"ok": True})
        except Exception as exc:
