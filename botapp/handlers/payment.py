...
async def deposit_from_menu(message: Message, state: FSMContext):
    ...
    # Ничего лишнего: WebApp открывается по нажатию кнопки в меню.
    # Отправляем только короткий фолбек, если окно не открылось.
    await message.answer(
        "Если страница оплаты не открылась автоматически, нажмите кнопку ниже.",
